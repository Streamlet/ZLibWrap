...
std::wstring ANSIToUCS2(const char *ansi, size_t length, UINT code_page) {
  ...
  if (size == 0)
    return result;
  result.resize(size);
  ::MultiByteToWideChar(code_page, 0, ansi, (int)length, (wchar_t *)result.data(), size);
  return result;
}
...
std::string UCS2ToANSI(const wchar_t *ucs2, size_t length, UINT code_page) {
  ...
  if (size == 0)
    return ansi;
  ansi.resize(size);
  ::WideCharToMultiByte(code_page, 0, ucs2, (int)length, (char *)ansi.data(), size, nullptr, nullptr);
  return ansi;
}
